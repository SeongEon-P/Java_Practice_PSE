...
//		blackPink.infoActivityArea();
		...
		// 하지만, 타입이 부모이기 때문에
		// 이 인스턴스를 실행을 하게되면, 바라보는 레퍼런스는 부모를 보게됩니다.
		// 무슨 의미냐, 부모의 기능만 구현이 된다는 뜻
		// 그래서 본인의 자식 클래스를 사용하고 싶다면, 다운캐스팅을 해줘야함
		// 부모 클래스 -> 자식 클래스로 다운 캐스팅 함
		
		Idol blackPink = new BlackPink(groupNameString, groupMember, groupAlbum, areaActivity);
		blackPink.introduceMember();
		blackPink.introduceAlbum();
		// 자식에서 기능을 하나를 더 추가 했지만, 객체로 점찍고 메서드를 호가인해보면
		// 자식 클래스에서 정의를 한 메서드가 안보임
		// 다형성을 이용해서, 사용을 하지만, 실제 사용은 부모것 밖에 사용을 못함.
		...
		// 상속에서, 모든 클래스들의 최고 부모 클래스는 Object 클래스.
		// 이걸 암묵적으로 다 상속을 받고 있음.
